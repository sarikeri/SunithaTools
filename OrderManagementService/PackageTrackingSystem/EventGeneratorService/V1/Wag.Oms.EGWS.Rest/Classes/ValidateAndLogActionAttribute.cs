    ...
    public class ValidateAndLogActionAttribute : ActionFilterAttribute
    {
        ...
        private void HandleModelValidationErrors(HttpActionContext actionContext, string referenceId)
        {
            if (!actionContext.ModelState.IsValid)
            {
                ...
                IEventGenerationResponse actionResponse = BuildErrorResponse(actionContext, errorMessage);
        ...
        private HttpContent GetObjectContent(HttpRequestMessage requestMessage, IEventGenerationResponse actionResponse)
        {
        ...
        {
            string errorMessage = (actionExecutedContext.Exception is EGWSException) ? ((EGWSException)(actionExecutedContext.Exception)).ErrorMessage : actionExecutedContext.Exception.Message;
            IEventGenerationResponse actionResponse = BuildErrorResponse(actionExecutedContext.ActionContext, errorMessage);
        ...
        private string GetReferenceId(HttpActionContext actionContext)
        {
            ...
            else if (actionContext.Request.Method == HttpMethod.Post || actionContext.Request.Method == HttpMethod.Put)
            {
                ...
                {
                    if (argument.Value is IEventGenerationRequest)
                    {
                        var request = (IEventGenerationRequest)argument.Value;
        ...
        private IEventGenerationResponse BuildErrorResponse(HttpActionContext actionContext, string errorMessage)
        {
            object response = Activator.CreateInstance(actionContext.ActionDescriptor.ReturnType);
            var actionResponse = response as IEventGenerationResponse;
            ...
            if (actionContext.Request.Method == HttpMethod.Post || actionContext.Request.Method == HttpMethod.Put)
            {
                ...
                {
                    if (requestObject is IEventGenerationRequest)
                    {
                        IEventGenerationRequest request = (IEventGenerationRequest)requestObject;
